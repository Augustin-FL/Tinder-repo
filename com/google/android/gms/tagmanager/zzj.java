...
class zzj extends zzdd {
    ...
    class C12641 implements zza {
        final /* synthetic */ Context zzrn;

        C12641(Context context) {
            this.zzrn = context;
        }
    ...
    public zzj(Context context) {
        this(context, new C12641(context));
    }
                ...
                if (zzl instanceof List) {
                    for (Object zzl2 : (List) zzl2) {
                        if (zzl2 instanceof Map) {
                            ...
                            zzbg.m1004e("ArbitraryPixel: additional params contains non-map: not sending partial hit: " + buildUpon.build().toString());
                            return;
                        }
                    }
                }
                zzbg.m1004e("ArbitraryPixel: additional params not a list: not sending partial hit: " + buildUpon.build().toString());
                return;
            }
            String uri = buildUpon.build().toString();
            this.zzaOP.zzzy().zzeH(uri);
            zzbg.m1005v("ArbitraryPixel: url = " + uri);
            if (zzg != null) {
                synchronized (zzj.class) {
                    zzaOO.add(zzg);
                    zzcv.zzb(this.mContext, zzaON, zzg, ServerProtocol.DIALOG_RETURN_SCOPES_TRUE);
                }
            }
        }
    }

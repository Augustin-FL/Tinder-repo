...
public final class GooglePlayServicesUtil {
    ...
    private static void zza(int i, Context context, String str) {
        ...
        if (zzi == null) {
            zzi = resources.getString(C0708R.string.common_google_play_services_notification_ticker);
        }
        ...
        if (zzlk.zzao(context)) {
            zzx.zzY(zzlv.zzpS());
            build = new Notification.Builder(context).setSmallIcon(C0708R.drawable.common_ic_googleplayservices).setPriority(2).setAutoCancel(true).setStyle(new BigTextStyle().bigText(zzi + " " + zzc)).addAction(C0708R.drawable.common_full_open_on_phone, resources.getString(C0708R.string.common_open_on_phone), errorPendingIntent).build();
        } else {
            CharSequence string = resources.getString(C0708R.string.common_google_play_services_notification_ticker);
            if (zzlv.zzpO()) {
                Notification build2;
    ...
    public static boolean zzh(Context context, String str) {
        ...
        try {
            if (context.getPackageManager().getApplicationInfo(str, Utility.DEFAULT_STREAM_BUFFER_SIZE).enabled) {
                return true;
            }
        } catch (NameNotFoundException e) {
        }

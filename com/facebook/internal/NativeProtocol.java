...
public final class NativeProtocol {
    ...
    static class C05471 implements Runnable {
        C05471() {
        }
    ...
    public static Intent createProxyAuthIntent(Context context, String str, Collection<String> collection, String str2, boolean z, boolean z2, DefaultAudience defaultAudience) {
        for (NativeAppInfo nativeAppInfo : facebookAppInfoList) {
            Intent putExtra = new Intent().setClassName(nativeAppInfo.getPackage(), FACEBOOK_PROXY_AUTH_ACTIVITY).putExtra(FACEBOOK_PROXY_AUTH_APP_ID_KEY, str);
            if (!Utility.isNullOrEmpty((Collection) collection)) {
                putExtra.putExtra(FACEBOOK_PROXY_AUTH_PERMISSIONS_KEY, TextUtils.join(",", collection));
            }
            if (!Utility.isNullOrEmpty(str2)) {
                putExtra.putExtra(FACEBOOK_PROXY_AUTH_E2E_KEY, str2);
            }
    ...
    public static Intent createProtocolResultIntent(Intent intent, Bundle bundle, FacebookException facebookException) {
        ...
        if (facebookException != null) {
            bundle2.putBundle(BRIDGE_ARG_ERROR_BUNDLE, createBundleForException(facebookException));
        }
    ...
    public static UUID getCallIdFromIntent(Intent intent) {
        ...
        if (intent != null) {
            ...
            if (isVersionCompatibleWithBucketedIntent(getProtocolVersionFromIntent(intent))) {
                ...
                if (bundleExtra != null) {
                    string = bundleExtra.getString(BRIDGE_ARG_ACTION_ID_STRING);
                } else {
                    Object obj = uuid;
                }
            } else {
    ...
    public static boolean isErrorResult(Intent intent) {
        ...
        if (bridgeArgumentsFromIntent != null) {
            return bridgeArgumentsFromIntent.containsKey(BRIDGE_ARG_ERROR_BUNDLE);
        }
    ...
    public static Bundle getErrorDataFromResultIntent(Intent intent) {
        ...
        Bundle bridgeArgumentsFromIntent = getBridgeArgumentsFromIntent(intent);
        if (bridgeArgumentsFromIntent != null) {
            return bridgeArgumentsFromIntent.getBundle(BRIDGE_ARG_ERROR_BUNDLE);
        }
        return intent.getExtras();
    }
    ...
    private static int getLatestAvailableProtocolVersionForAppInfoList(List<NativeAppInfo> list, int[] iArr) {
        ...
        for (NativeAppInfo availableVersions : list) {
            ...
            if (computeLatestAvailableVersionFromVersionSpec != NO_PROTOCOL_AVAILABLE) {
    ...
    public static void updateAllAvailableProtocolVersionsAsync() {
        if (protocolVersionsAsyncUpdating.compareAndSet(false, true)) {
            FacebookSdk.getExecutor().execute(new C05471());
        }
    }
    ...
    private static TreeSet<Integer> fetchAllAvailableProtocolVersionsForAppInfo(NativeAppInfo nativeAppInfo) {
        ...
        String[] strArr = new String[]{PLATFORM_PROVIDER_VERSION_COLUMN};
            ...
            if (FacebookSdk.getApplicationContext().getPackageManager().resolveContentProvider(nativeAppInfo.getPackage() + PLATFORM_PROVIDER, 0) != null) {
                ...
                if (query != null) {
                    while (query.moveToNext()) {
                        try {
                            treeSet.add(Integer.valueOf(query.getInt(query.getColumnIndex(PLATFORM_PROVIDER_VERSION_COLUMN))));
                        } catch (Throwable th2) {
                            cursor = query;
                            th = th2;
                        }
    ...
    public static int computeLatestAvailableVersionFromVersionSpec(TreeSet<Integer> treeSet, int i, int[] iArr) {
        int length = iArr.length + NO_PROTOCOL_AVAILABLE;
        Iterator descendingIterator = treeSet.descendingIterator();
        int i2 = NO_PROTOCOL_AVAILABLE;
        int i3 = length;
